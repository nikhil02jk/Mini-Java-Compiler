...
ids1: IDENTIFIER EQ arithm_e    {if(mapping($1)==-1){idx++; strcpy(st[idx].idName,$1); st[idx].type =0; st[idx].value = $3;st[idx].line_no=yylineno;st[idx].scope=n;} }
   | ids1 COMMA IDENTIFIER    {if(mapping($3)==-1) {idx++; strcpy(st[idx].idName,$3);  st[idx].type =0;st[idx].line_no=yylineno;st[idx].scope=n; } else {yyerror(errMess);} }
   | IDENTIFIER    	 {if(mapping($1)==-1) {idx++; strcpy(st[idx].idName,$1); st[idx].type =0;st[idx].line_no=yylineno;st[idx].scope=n; } }
   | IDENTIFIER EQ rel_e    {xpos = mapping($1);  if(xpos==-1) {idx++; strcpy(st[idx].idName,$1); st[idx].type =3; st[idx].value = $3;st[idx].line_no=yylineno;st[idx].scope=n;} else { st[xpos].value = $3; } }
   ;

ids2: IDENTIFIER EQ arithm_e	{if(mapping($1)==-1) {idx++; strcpy(st[idx].idName,$1); st[idx].type =1; st[idx].value = $3;st[idx].line_no=yylineno;st[idx].scope=n;} }
   | ids2 COMMA IDENTIFIER    {if(mapping($3)==-1) { idx++; strcpy(st[idx].idName,$3);  st[idx].type =1;st[idx].line_no=yylineno; st[idx].scope=n;} else {yyerror(errMess);}}
   | IDENTIFIER   		 {if(mapping($1)==-1) {idx++; strcpy(st[idx].idName,$1);  st[idx].type =1; st[idx].line_no=yylineno;st[idx].scope=n;}}
   ;

ids3: IDENTIFIER EQ rel_e    {if(mapping($1)==-1) {idx++; strcpy(st[idx].idName,$1); st[idx].type =3; st[idx].value = $3;} else { st[xpos].value = $3; st[idx].line_no=yylineno;st[idx].scope=n;} }
  // | ids3 COMMA IDENTIFIER    {if(mapping($3)==-1) {idx++; strcpy(st[idx].idName,$3);  st[idx].type =3; } else {printf(yyerror(errMess));} }
   | IDENTIFIER    	 {if(mapping($1)==-1) {idx++; strcpy(st[idx].idName,$1); st[idx].type =3;st[idx].line_no=yylineno; st[idx].scope=n;} }
   ;
...
rel_e: arithm_e LT arithm_e   	 {$$ = func($1,$3,1);}
      ...
      | IDENTIFIER EQ rel_e     {xpos = mapping($1);if(xpos!=-1) { st[xpos].value = $3; st[xpos].line_no=yylineno;st[xpos].scope=n;} else { yyerror(errMess);}}
      	| TRUE1   		 {$$=1;}
      	| FALSE1   		 {$$=0;}
      	;
...
arithm_e: arithm_e MUL arithm_e   	 {$$ = $1 * $3;}
     ...
     | IDENTIFIER   		{xpos = mapping($1); if(xpos!=-1) { $$ = st[xpos].value;  st[xpos].line_no=yylineno;st[xpos].scope=n;} else {yyerror(errMess);} }
     | NUM   			 {$$ =  $1; }
    // | DECIMAL   		 {$$ =  $1; }
     | IDENTIFIER INC_OP   	 {xpos = mapping($1); $$=st[xpos].value+1; st[xpos].line_no=yylineno;st[xpos].scope=n;}
     | IDENTIFIER DEC_OP   	 {xpos = mapping($1); $$=st[xpos].value-1; st[xpos].line_no=yylineno;st[xpos].scope=n;}
     | INC_OP IDENTIFIER    	 {xpos = mapping($2); $$=st[xpos].value+1; st[xpos].line_no=yylineno;st[xpos].scope=n;}
     | DEC_OP IDENTIFIER    	 {xpos = mapping($2); $$=st[xpos].value-1; st[xpos].line_no=yylineno;st[xpos].scope=n;}
     | IDENTIFIER EQ arithm_e    {xpos = mapping($1); if(xpos!=-1) { st[xpos].value = $3;  st[xpos].line_no=yylineno;st[xpos].scope=n;} else {yyerror(errMess);}}
	//| rel_e			{$$ = $1;}     
	;
